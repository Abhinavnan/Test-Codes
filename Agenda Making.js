...
class App extends Component {
  ...
  topicFun = () => {
    this.setState(prevState => ({
      topicsArr: [...prevState.topicsArr, this.state.newTopic],
      newTopic: "",
    }));
  };
            ...
            <form onSubmit={this.formChec}>
              ...
              <div className="my-3 w-50">
                ...
                <small className="text-danger" datatestid="invalidTopic">
                  {this.state.newTopic.trim().length === 0 && this.state.topicsArr.length === 0 ? "Topic is required" : ""}
                </small>
              </div>
              <button
              ...
              <button
                ...
                disabled={this.state.newTitle.trim().length === 0 || this.state.newDescription.trim().length === 0 || this.state.topicsArr.length === 0}
              >
                Submit Agenda
              </button>
            </form>
          ...
          <div className="container" role="viewAgenda">
            ...
            {this.state.AddedAgenda.map((agenda, index) => (
              <div className="card my-3" role="cards" key={index}>
                ...
                <div className="card-body">
                  <ul className="list-group">
                    {agenda.topics.map((topic, innerIndex) => (
                      <li className="list-group-item" key={innerIndex}>
                        {topic}
                      </li>
                    ))}
                  </ul>
